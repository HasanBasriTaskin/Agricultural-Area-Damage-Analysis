...
export default function JobsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "failed":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-red-500/15 text-red-400 border border-red-500/30">
            <XCircle className="w-3.5 h-3.5" /> Başarısız
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30 animate-pulse">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="bg-zinc-900 border border-zinc-800 rounded-xl pl-9 pr-4 py-2 text-xs text-zinc-100 focus:outline-none focus:border-emerald-500 transition-colors w-64"
              />
            </div>
            <button
              onClick={fetchJobs}
        ...
        <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl backdrop-blur-md">
          {loading ? (
            ...
            <div className="p-12 text-center space-y-3">
              ...
              <p className="text-sm font-medium text-zinc-300">Henüz kaydedilmiş analiz bulunamadı.</p>
              <p className="text-xs text-zinc-500">Harita üzerinden yeni bir alan seçip analizi başlatabilirsiniz.</p>
              <Link
                href="/"
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold transition-all mt-2"
              >
                Yeni Analiz Başlat
              </Link>
            </div>
                      ...
                      <td className="py-3 px-4 text-right space-x-2">
                        {job.status === "done" && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-[11px] font-semibold border border-zinc-700 transition-all"
                          >
                        ...
                        <Link
                          href={`/?jobId=${job.id}`}
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 text-[11px] font-semibold border border-emerald-500/30 transition-all"
                        >
                          Görüntüle
