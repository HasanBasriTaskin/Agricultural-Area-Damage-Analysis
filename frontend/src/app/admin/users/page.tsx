      ...
      <div className="min-h-screen flex bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 transition-colors">
        ...
        <main className="flex-1 p-12 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <p className="text-xs text-zinc-600 dark:text-zinc-400 max-w-md">
            Kullanıcı yönetimi paneline sadece <b className="text-amber-600 dark:text-amber-400">ADMIN</b> rolündeki yöneticiler erişebilir. Lütfen yetkili bir hesapla oturum açınız.
          </p>
        </main>
      </div>
    );
      ...
      <main className="flex-1 p-8 overflow-y-auto max-w-7xl mx-auto space-y-6">
                  ...
                  {users.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                        <td className="py-3.5 px-4 text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-zinc-200 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 flex items-center justify-center text-[10px] font-bold text-zinc-700 dark:text-zinc-300">
                            {user.email.substring(0, 2).toUpperCase()}
                          </div>
                          <span>{user.email}</span>
                          {isSelf && (
                            <span className="text-[10px] bg-amber-500/20 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-mono font-bold">
                              Sen
                            </span>
                          )}
                        </td>
                        <td className="py-3.5 px-4">
                          <select
                            ...
                            disabled={isSelf}
                            className={`border rounded-lg px-2.5 py-1 text-xs font-mono font-bold cursor-pointer transition-colors ${
                              user.role === "admin"
                                ? "text-red-700 dark:text-red-400 border-red-500/40 bg-red-500/10"
                                : user.role === "analyst"
                                ? "text-blue-700 dark:text-blue-400 border-blue-500/40 bg-blue-500/10"
                                : "text-zinc-700 dark:text-zinc-400 border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900"
                          ...
                          <button
                            ...
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold transition-all cursor-pointer ${
                              user.is_active
                                ...
                                : "bg-zinc-200 dark:bg-zinc-800 text-zinc-500 border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-700/50"
                            } disabled:opacity-60 disabled:cursor-not-allowed`}
                          >
                        ...
                        <td className="py-3.5 px-4 text-right">
                          <button
                            ...
                            disabled={isSelf}
                            className="p-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 hover:bg-red-500/20 text-zinc-500 dark:text-zinc-400 hover:text-red-600 dark:hover:text-red-400 border border-zinc-300 dark:border-zinc-700 hover:border-red-500/30 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed shadow-sm"
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl transition-colors">
              ...
              <form onSubmit={handleCreateUser} className="space-y-3">
                <div>
                  <label className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">E-posta</label>
                  <input
                    type="email"
                    required
                    value={newEmail}
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-zinc-900 dark:text-zinc-100 mt-1 focus:outline-none focus:border-emerald-500"
                  />
                </div>

                <div>
                  <label className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Şifre</label>
                  <input
                    type="password"
                    required
                    value={newPassword}
                ...
                <div>
                  <label className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Rol</label>
                  <select
                    value={newRole}
                ...
                <div className="flex items-center justify-end gap-2 pt-3">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="px-4 py-2 rounded-xl bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-xs font-semibold text-zinc-800 dark:text-zinc-300 cursor-pointer"
                  >
                    İptal
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-xs font-semibold text-white cursor-pointer shadow-lg shadow-emerald-950/20"
                  >
                    Kaydet
                  </button>
                </div>
